...
function LeftSideBarWeb() {
  ...
  return (
    <aside className="bg-white sticky top-0 w-[200px] hidden flex-shrink-0 md:flex flex-col gap-4">
      ...
      <div className="fixed flex flex-col gap-4">
        ...
        <div className="w-full relative">
          <div
            className="p-1 max-w-[63%] bg-customBlueFont text-white rounded-lg cursor-pointer hover:"
            onClick={() =>
              dispatch({ ...state, showDropDown: !state.showDropDown })
            }
          >
          ...
          </div>
          {
            ...
            >
              <p
                className="indent-3 p-1 text-customBlueFont border-b hover:text-customBlueFont hover:bg-orange-100  flex items-center justify-between pr-6"
                onClick={() =>
                  dispatch({ selectedValue: "A", showDropDown: false })
                }
              >
                Type A
              </p>
              <p
                className="indent-3 p-1 text-customBlueFont hover:text-customBlueFont hover:bg-orange-100  flex items-center justify-between pr-6"
                onClick={() =>
                  dispatch({ selectedValue: "B", showDropDown: false })
                }
              >
                Type B
              </p>
              ...
              if (ele.type === "TradingAPI")
                return (
                  <Link
                    to={`/developer/type-${
                      state.selectedValue
                    }/${ele.title.replaceAll(" ", "-")}`.toLowerCase()}
                  >
                    <li
                      className={`${
                        ele.path.split("/")[2] ===
